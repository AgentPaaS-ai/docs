...
- Cron uses named intervals (for example `every_5m`), not free-form cron expressions.
- `cloud pull` returns project metadata for the edit loop; keep a local pack for full source when needed.
- Trial access is invite-based via a claim link from [agentpaas.ai](https://agentpaas.ai/).
